...
function loginJugador(){ 
  $("#facil").on("click", function() { 
	...
	jugadorInfo.dni = $('#dni').val();
	jugadorInfo.tel = $('#tel').val();
...
function armarTabla() { //armo la tabla que mostrará el ranking de ganadores (nombre, nivel y número de intentos)
  ...
  let cabecera = '<th>Nombre</th><th>Nivel</th><th>DNI</th><th>Tel</th><th>Intentos</th>'
...
function armarRanking() { 
  ...
  for (let i = 0; i < infoJugador.length; i++) {
	...
	let dniInfo = "<td>" + infoJugador[i].dni + "</td>";
	let telInfo = "<td>" + infoJugador[i].tel + "</td>";
  ...
  $('.volver-jugar').on('click', function () {
    ...
    location.reload();
        ...
        function descargarJSON() {
            ...
            const archivo = new Blob([JSON.stringify(jugador)], { type: 'application/json' });
            const enlace = document.createElement('a');
            enlace.href = URL.createObjectURL(archivo);
            enlace.download = 'jugador.json';
